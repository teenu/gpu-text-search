class GpuTextSearch < Formula
  ...
  def install
    ...
    # Install the Metal resource bundle alongside the executable (where Swift expects it)
  ...
  def post_install
    # Swift's Bundle.main.bundleURL resolves to symlink location in Homebrew
    # Copy the bundle to where the symlinked executable can find it
    bundle_source = bin/"GPUTextSearch_SearchEngine.bundle"
    bundle_target = HOMEBREW_PREFIX/"bin/GPUTextSearch_SearchEngine.bundle"
    
    if bundle_source.exist? && !bundle_target.exist?
      cp_r bundle_source, bundle_target
    end
  end
